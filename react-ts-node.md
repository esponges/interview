...
- What's the difference between __SSR and CSR__? What are the pros and cons of each one?

- What's __SSG__ is it related to SSR? How?

> Static Site Generation (SSG) is the process of generating a static HTML page at build time. It's related to SSR because it's used to generate the HTML of the page at build time. It's used to improve performance and SEO. It's used when the content of the page doesn't change often.

- What's __hydration__? How is it related to SSR?

> Hydration is the process of attaching event listeners to the HTML generated by SSR. When the client receives the HTML generated by SSR it will use these listeners to add interactivity to the page.

- What's a PWA? 

> A PWA is a Progressive Web App. It's a web application that uses modern web capabilities to deliver an app-like experience to users. When the application doesn't have a native mobile app it could be used as a temporary solution or permanent if the application doesn't require mobile features.
